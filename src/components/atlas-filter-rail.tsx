...
// Maps each known region to a continent bucket for grouped display.
...
export type FilterLocation = ReefLocationCardData & {
  ...
  // Geo carried through so the explorer can build globe markers from results.
  lat: number;
  lng: number;
  countryCode: string;
};
...
export type Filters = {
  ...
  skill: string[];
  region: string[];
  heat: string[]; // buckets: low (0-1), elevated (2), alert (3-5)
  freshOnly: boolean;
  sort: SortKey;
};
...
const HEAT_BUCKETS: { value: string; label: string; hint: string; test: (h: number) => boolean }[] = [
  { value: "low", label: "Low stress", hint: "No stress / watch", test: (h) => h <= 1 },
...
export function applyFilters(locs: FilterLocation[], f: Filters): FilterLocation[] {
  const filtered = locs.filter((r) => {
    ...
    if (f.skill.length && !f.skill.includes(r.skill)) return false;
    ...
    if (f.freshOnly) {
      // "Needs fresh eyes" — surveys that are stale or cold, or never surveyed.
      const k: FreshnessKey | "none" = r.lastSurveyDays === null ? "none" : freshness(r.lastSurveyDays).k;
  ...
  switch (f.sort) {
    case "oldest":
      sorted.sort((a, b) => (b.lastSurveyDays ?? -1) - (a.lastSurveyDays ?? -1));
      break;
    case "heat":
      sorted.sort((a, b) => b.heatLevel - a.heatLevel);
      break;
    case "name":
    ...
    case "season":
    default:
      sorted.sort((a, b) => firstBestMonth(a.bestMonths) - firstBestMonth(b.bestMonths));
      break;
  }
  return sorted;
}
...
function RegionFilter({
  ...
  // Group regions by continent, preserving CONTINENT_ORDER.
  ...
  // Start all continents collapsed; open ones that have active selections.
  const [open, setOpen] = useState<Record<string, boolean>>(() => {
    const init: Record<string, boolean> = {};
    for (const c of continents) {
      init[c] = (grouped[c] ?? []).some((r) => selected.includes(r));
    }
    return init;
  });

  const toggle = (c: string) => setOpen((prev) => ({ ...prev, [c]: !prev[c] }));
          ...
          return (
            <div key={continent}>
              <button
                ...
                className="flex w-full items-center justify-between rounded-lg px-2.5 py-1.5 text-left text-sm font-medium text-slate-700 hover:bg-slate-50"
              >
                <span className="flex items-center gap-2">
                  <span>{continent}</span>
                  {activeCount > 0 && (
...
/**
 * Presentational filter rail. Owns no state — the explorer holds the shared
 * Filters and feeds them down, receiving every change through onChange.
 */
export function AtlasFilterRail({
  ...
  const toggle = (
    facet: "condition" | "months" | "skill" | "region" | "heat",
    value: string | number,
  ) => {
    ...
    <aside className="lg:sticky lg:top-24 lg:self-start">
      <div className="space-y-4 rounded-2xl border border-slate-200 bg-white p-5">
        ...
        <FacetGroup title="Evidence gaps">
          <CheckOpt
            on={f.freshOnly}
            onClick={() => onChange({ ...f, freshOnly: !f.freshOnly })}
          >
            Needs fresh eyes
          </CheckOpt>
        ...
        <FacetGroup title="Thermal stress">
          {HEAT_BUCKETS.map((b) => (
            <CheckOpt key={b.value} on={f.heat.includes(b.value)} onClick={() => toggle("heat", b.value)}>
                ...
                <button
                  ...
                  className={`rounded-lg px-1 py-1.5 text-xs font-medium transition ${
                    on
                      ...
                      : "bg-slate-50 text-slate-600 hover:bg-[#e8f0fe]"
                  }`}
                >
                  {m}
                </button>
        ...
        {regions.length > 0 && (
          <RegionFilter regions={regions} selected={f.region} onToggle={(r) => toggle("region", r)} />
        )}
      </div>
    </aside>
  );
