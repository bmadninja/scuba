...
const SKILL_TEXT_RANK: Record<string, number> = {
  Beginner: 0,
  "Open water+": 1,
  "Advanced+": 2,
  Technical: 3,
...
const WHAT_TO_SEE_GROUPS: {
...
}[] = [
  {
    ...
    items: [
      { value: "Whale sharks", label: "Whale sharks" },
      { value: "Hammerheads", label: "Hammerheads" },
      { value: "Sharks", label: "Reef sharks" },
      { value: "Rays & mantas", label: "Rays & mantas" },
      { value: "Eagle rays", label: "Eagle rays" },
    ],
  },
  {
    ...
    items: [
      ...
      { value: "Seals & sea lions", label: "Seals & sea lions" },
      { value: "Dugongs", label: "Dugongs" },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      { value: "Cephalopods", label: "Octopus & cuttlefish" },
      { value: "Frogfish & seahorses", label: "Frogfish & seahorses" },
      { value: "Nudibranchs", label: "Nudibranchs" },
      { value: "Corals & inverts", label: "Corals & inverts" },
    ],
  },
];
...
// ─── Region grouped sub-items (same as sites-explorer) ───────────────────────
...
function regionFilterMatches(filterValue: string, region: string): boolean {
  if (BROAD_REGION_VALUES.has(filterValue)) {
    return (REGION_BUCKET[region] ?? "Other") === filterValue;
  }
...
export function LocationsExplorer({ locations, currentMonth }: Props) {
  ...
  const certFilters = getMultiParam(params, "cert");
  ...
  const clearAll = () =>
    router.replace(viewMode === "map" ? "/locations?view=map" : "/locations", { scroll: false });

  const filtered = useMemo(() => {
    const q = query.trim().toLowerCase();
    let result = locations.filter((loc) => {
      if (reefStates.length > 0 && !reefStates.includes(loc.state)) return false;
      if (certFilters.length > 0) {
        const maxRank = Math.max(...certFilters.map((c) => SKILL_TEXT_RANK[c] ?? 0));
        if ((SKILL_TEXT_RANK[loc.skill] ?? 0) > maxRank) return false;
      }
      if (seeFilters.length > 0 && !seeFilters.some((f) => loc.animalTags.includes(f))) return false;
      if (regionFilters.length > 0 && !regionFilters.some((f) => regionFilterMatches(f, loc.region))) return false;
      if (freshFilter && loc.lastSurveyDays !== null && loc.lastSurveyDays <= 365) return false;
      if (monthFilters.length > 0 && !monthFilters.some((m) => loc.bestMonths.includes(m))) return false;
      if (q) {
        const hay = [loc.name, loc.hook, loc.country, loc.region, ...loc.animalTags]
          .join(" ")
          .toLowerCase();
    ...
    if (sortKey === "name") {
      ...
      result = [...result].sort((a, b) => {
        const da = a.lastSurveyDays ?? 9999;
        const db = b.lastSurveyDays ?? 9999;
        return db - da;
      });
    } else {
      result = [...result].sort((a, b) => {
    ...
    return result;
  }, [locations, query, reefStates, certFilters, seeFilters, regionFilters, freshFilter, monthFilters, sortKey, currentMonth]);
    ...
    <>
      ...
      <header className="le-header" style={{ background: "#0b1e32", padding: "4rem 3rem 5rem" }}>
        <div style={{ maxWidth: 1320, margin: "0 auto" }}>
          ...
          <div style={{ position: "relative", maxWidth: 480 }}>
            <svg style={{ position: "absolute", left: "1rem", top: "50%", transform: "translateY(-50%)", color: "rgba(255,255,255,0.35)", pointerEvents: "none" }} width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round">
            ...
            <input
              ...
              placeholder="Search by location name, animal, or region…"
              style={{ width: "100%", background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.14)", borderRadius: 999, padding: "0.75rem 1.25rem 0.75rem 2.75rem", fontSize: "0.9375rem", fontFamily: "inherit", color: "rgba(255,255,255,0.7)", outline: "none" }}
            />
          </div>
        </div>
      </header>
      ...
      {hasActiveFilter && (
        <div className="le-filter-summary" style={{ maxWidth: "100%", borderBottom: "1px solid rgba(255,255,255,0.1)", background: "#0a1628", padding: "0.625rem 3rem" }}>
          <div style={{ maxWidth: 1320, margin: "0 auto", display: "flex", alignItems: "center", gap: "0.5rem", flexWrap: "wrap" }}>
            ...
            {seeFilters.map((f) => <ActivePill key={f} label={getSeeLabel(f)} onRemove={() => toggleMultiParam("see", f)} />)}
            {regionFilters.map((f) => <ActivePill key={f} label={getRegionLabel(f)} onRemove={() => toggleMultiParam("region", f)} />)}
            {monthFilters.map((m) => <ActivePill key={m} label={MONTH_LABELS[m - 1]} onRemove={() => toggleMultiParam("month", String(m))} />)}
            {certFilters.map((c) => <ActivePill key={c} label={CERT_OPTIONS.find((o) => o.value === c)?.label ?? c} onRemove={() => toggleMultiParam("cert", c)} />)}
            {freshFilter && <ActivePill label="Fresh eyes" onRemove={() => setParam("fresh", null)} />}
            {query && <ActivePill label={`"${query}"`} onRemove={() => setParam("q", null)} />}
            <button type="button" onClick={clearAll} style={{ marginLeft: "auto", fontSize: "0.75rem", color: "#64748b", background: "none", border: "none", cursor: "pointer", fontFamily: "inherit" }}>
              Clear all
            </button>
          </div>
        </div>
      ...
      {viewMode === "map" && (
        <div style={{ maxWidth: 1320, margin: "0 auto" }} className="px-5 sm:px-8 lg:px-12">
          <LocationsGlobe locations={filtered} />
        </div>
      )}
...
function LocationCard({
  ...
  return (
    <Link
      ...
      style={{ borderRadius: "1.25rem", overflow: "hidden", border: "1px solid rgba(255,255,255,0.1)", background: "#0a1628", textDecoration: "none", color: "inherit", display: "flex", flexDirection: "column", boxShadow: "0 1px 2px rgba(16,40,70,.03), 0 8px 24px -12px rgba(16,40,70,.09)", transition: "transform 0.2s, box-shadow 0.2s, border-color 0.2s" }}
    ...
    >
      ...
      <div style={{ padding: "1.25rem 1.375rem 1.375rem", flex: 1, display: "flex", flexDirection: "column" }}>
        ...
        <div style={{ display: "flex", gap: "0.375rem", flexWrap: "wrap", marginTop: "0.875rem", paddingTop: "0.875rem", borderTop: "1px solid rgba(255,255,255,0.1)" }}>
          {location.animalTags.slice(0, 3).map((tag) => (
            <span key={tag} style={{ fontSize: "0.5875rem", fontWeight: 600, padding: "0.2rem 0.55rem", borderRadius: 999, background: "rgba(0,212,255,0.12)", color: "#00d4ff", textTransform: "capitalize" }}>
              {tag}
            </span>
          ))}
          {location.diveTypeTags.slice(0, 1).map((t) => (
            <span key={t} style={{ fontSize: "0.5875rem", fontWeight: 600, padding: "0.2rem 0.55rem", borderRadius: 999, background: "rgba(255,255,255,0.05)", color: "#8b9db8", textTransform: "capitalize" }}>
              {t.replace(/-/g, " ")}
            </span>
          ))}
        </div>
...
function FilterDropdown({ label, selectedCount, children }: { label: string; selectedCount: number; children: React.ReactNode }) {
  ...
  return (
    <div ref={ref} style={{ position: "relative", display: "inline-block" }}>
      <button type="button" onClick={() => setOpen((o) => !o)} style={{ display: "inline-flex", alignItems: "center", gap: "0.35rem", padding: "0.45rem 0.9375rem", borderRadius: 999, border: `1px solid ${isActive || open ? "#00d4ff" : "rgba(255,255,255,0.1)"}`, background: isActive || open ? "rgba(0,212,255,0.12)" : "rgba(255,255,255,0.05)", fontSize: "0.8125rem", fontWeight: isActive ? 600 : 500, color: isActive || open ? "#00d4ff" : "#8b9db8", cursor: "pointer", fontFamily: "inherit", whiteSpace: "nowrap", transition: "all 0.15s" }}>
        {label}
        {isActive && <span style={{ fontSize: "0.625rem", background: "#00d4ff", color: "#030712", borderRadius: 999, padding: "0.1rem 0.45rem", fontWeight: 700, lineHeight: 1.4 }}>{selectedCount}</span>}
      ...
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 0.375rem)", left: 0, zIndex: 50, background: "#0a1628", border: "1px solid rgba(255,255,255,0.1)", borderRadius: "0.625rem", boxShadow: "0 4px 24px -4px rgba(0,0,0,0.5)", padding: "0.375rem", minWidth: 220, maxHeight: 420, overflowY: "auto" }}>
          {children}
        </div>
      )}
    </div>
...
function ExpandableGroup({ groupLabel, items, selectedValues, onToggle, defaultExpanded = false }: { groupLabel: string; items: { value: string; label: string }[]; selectedValues: string[]; onToggle: (value: string) => void; defaultExpanded?: boolean }) {
  ...
  return (
    <div>
      <button type="button" onClick={() => setExpanded((e) => !e)} style={{ display: "flex", alignItems: "center", gap: "0.5rem", width: "100%", textAlign: "left", padding: "0.45rem 0.875rem", fontSize: "0.8125rem", fontWeight: 600, fontFamily: "inherit", background: selectedInGroup > 0 ? "rgba(0,212,255,0.06)" : "transparent", color: selectedInGroup > 0 ? "#00d4ff" : "#aebcd0", border: "none", cursor: "pointer", borderRadius: "0.375rem" }}>
        <svg width="8" height="8" viewBox="0 0 8 8" fill="none" style={{ flexShrink: 0, transform: expanded ? "rotate(90deg)" : "none", transition: "transform 0.15s" }}>
          <path d="M2 1.5L5.5 4L2 6.5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
        {groupLabel}
        {selectedInGroup > 0 && <span style={{ marginLeft: "auto", fontSize: "0.625rem", background: "rgba(0,212,255,0.2)", color: "#00d4ff", padding: "0.1rem 0.45rem", borderRadius: 999, fontWeight: 700, lineHeight: 1.4 }}>{selectedInGroup}</span>}
      </button>
      ...
      {expanded && (
        <div style={{ paddingBottom: "0.25rem" }}>
          {items.map((item) => {
            ...
            return (
              <button key={item.value} type="button" onClick={(e) => { e.stopPropagation(); onToggle(item.value); }} style={{ display: "flex", alignItems: "center", gap: "0.5rem", width: "100%", textAlign: "left", paddingLeft: "1.875rem", paddingRight: "0.875rem", paddingTop: "0.375rem", paddingBottom: "0.375rem", fontSize: "0.8125rem", fontFamily: "inherit", background: isSelected ? "rgba(0,212,255,0.1)" : "transparent", color: isSelected ? "#00d4ff" : "#f0f4f8", border: "none", cursor: "pointer", borderRadius: "0.375rem" }}>
                <CheckBox checked={isSelected} />
                {item.label}
              </button>
            );
...
function CheckBox({ checked }: { checked: boolean }) {
  return (
    <span style={{ width: 14, height: 14, borderRadius: 3, border: `1.5px solid ${checked ? "#00d4ff" : "rgba(255,255,255,0.25)"}`, background: checked ? "#00d4ff" : "transparent", display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "background 0.1s, border-color 0.1s" }}>
      {checked && <svg width="8" height="8" viewBox="0 0 8 8" fill="none"><path d="M1 4l2 2 4-4" stroke="#030712" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>}
    </span>
  );
}
