...
// want to see?" hero copy with a static marketing hero (src/app/page.tsx).
// The interactive explorer — including a "What to see" filter pill — now
// lives at /locations (src/components/explore-page.tsx).
test.describe('Homepage copy changes', () => {
  test('hero shows the redesigned headline', async ({ page }) => {
    await page.goto('/', GOTO);
    // The live homepage hero h1 (src/app/page.tsx).
    await expect(
  ...
  test('"What to see" exists as a filter dropdown-pill button on /locations', async ({ page }) => {
    await page.goto('/locations', GOTO);
    await expect(
      page.getByRole('button', { name: 'What to see', exact: true }).first(),
    ).toBeVisible({ timeout: 15_000 });
  });
...
// The filter rail lives on /locations as a row of dropdown-pill buttons
// (aria-expanded), not <details>/<summary> sections, and has no (i) info
// buttons.
test.describe('Atlas filter rail', () => {
  test('"Certification" is a filter dropdown-pill button on /locations', async ({ page }) => {
    await page.goto('/locations', GOTO);
    await expect(
      page.getByRole('button', { name: 'Certification', exact: true }).first(),
    ).toBeVisible({ timeout: 15_000 });
  });
});
