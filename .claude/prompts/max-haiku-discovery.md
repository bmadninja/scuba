...
2. **Find gaps:** `node scripts/find-site-gaps.mjs 3`
   This prints up to 3 target locations (empty locations first, then thin ones). **If it prints `[]`, the catalog is caught up — stop now, this run is a cheap no-op. Do not invent work.**
